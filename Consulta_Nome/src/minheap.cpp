...
MinHeap::MinHeap(Logradouro** arr, int n) {//construtor da Heap
    tamanho=n;
    capacidade=n;
    vetor = new Logradouro*[n];
...
MinHeap::MinHeap(int capacidadeMax) :tamanho(0), capacidade(capacidadeMax) {
    vetor = new Logradouro*[capacidadeMax];
}

void MinHeap::Troca(int i, int j) {//função de troca
    Logradouro* temp = vetor[i];
    vetor[i] = vetor[j];
    vetor[j] = temp;
}
...
void MinHeap::ajustar_para_baixo(int no_pai) {
    ...
    if (indice_esquerda < tamanho && vetor[indice_esquerda]->distanciaCache < vetor[no_pai]->distanciaCache)
        menor_indice = indice_esquerda;
    if (indice_direita < tamanho && vetor[indice_direita]->distanciaCache < vetor[menor_indice]->distanciaCache)
...
void MinHeap::ajustar_para_cima(int no_filho) {
    ...
    while (no_filho > 0 && vetor[no_filho]->distanciaCache < vetor[no_pai]->distanciaCache) {//verifica se o filho é menor que o pai
...
void MinHeap::inserir(Logradouro* obj) {//insere no final da heap o novo elemento
...
Logradouro* MinHeap::extrair_minimo(){
    if(tamanho == 0) return nullptr;
    Logradouro* raiz = vetor[0];
